...
# Функція для побудови та оцінки класифікатора для різних ядер
def evaluate_svm_kernel(kernel_type, degree=3):
    print(f"Оцінка для ядра: {kernel_type}")
    if kernel_type == 'poly':
...
# Поліноміальне ядро
evaluate_svm_kernel('poly', degree=2)
